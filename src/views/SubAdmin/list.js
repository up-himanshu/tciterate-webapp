...
class SubAdmins extends React.Component {
	...
	_fetchListData = () => {
		APIService.fetchAllSubAdmins().then(
			(units) => {
				this.setState({
					loginStatus: true,
					listItems: true,
	...
	_handleDelete(id) {
		APIService.deleteSubAdmin(id).then(
			() => {
				this.setState({
					listItems: true,
					loading: false
	...
	_handleStatus(id) {
		APIService.statusAdmin(id).then(
			() => {
				this.setState({
					listItems: true,
					loading: false
			...
			return (
				<div>
					...
					<Container fluid className="main-content-container px-4">
						<MainTitle title="Sub Admins Section" />
						<Row>
							<Col>
										...
										<CardBody className="p-0 pb-3">
											<MaterialTable
												...
												columns={[
													...
													{ title: 'Role Type', field: 'role_types.role_type' }
												]}
												...
												actions={[
													{
														...
														onClick: (event, rowData) => {
															this.setState({
																redirect: true,
																redirectPath: '/sub-admins/' + rowData.id,
													...
													},
													{
														...
														onClick: (event, rowData) => {
															this.setState({
																redirect: true,
																redirectPath: '/sub-admins/' + rowData.id + '/edit',
																redirectData: {
																	data: rowData,
																	id: rowData.id,
																	update: true,
