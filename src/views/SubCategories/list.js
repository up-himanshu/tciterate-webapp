...
class SubCategories extends React.Component {
	...
	_fetchListData = () => {
		APIService.fetchAllSubCategories().then(
			(units) => {
				this.setState({
					loginStatus: true,
					listItems: true,
	...
	_handleDelete(id) {
		APIService.deleteSubCategory(id).then(
			() => {
				this.setState({
					listItems: true,
					loading: false
	...
	_handleStatus(id) {
		APIService.statusSubCategory(id).then(
			() => {
				this.setState({
					listItems: true,
					loading: false
									...
									<ContentHeader>
										<Button
											...
											onClick={() =>
												this.setState({
													redirect: true,
													redirectPath: '/sub-categories/new'
												})
											}
										>
											Add Sub-Category
										...
										<CardBody className="p-0 pb-3">
											<MaterialTable
												...
												columns={[
													...
													{ title: 'Sub-Category Name', field: 'sub_category_name' },
													{ title: 'Sub-Category Description', field: 'description' }
												]}
												...
												actions={[
													{
														...
														onClick: (event, rowData) => {
															this.setState({
																redirect: true,
																redirectPath: '/sub-categories/' + rowData.id,
																redirectData: {
																	name: rowData.sub_category_name,
													...
													},
													{
														...
														onClick: (event, rowData) => {
															this.setState({
																redirect: true,
																redirectPath: '/sub-categories/' + rowData.id + '/edit',
																redirectData: {
																	name: rowData.sub_category_name,
																	id: rowData.id,
